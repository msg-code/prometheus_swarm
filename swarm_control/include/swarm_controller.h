...
int target_id;                                  // 围捕目标ID
string neighbour_name1,neighbour_name2;         // 邻居名字
string target_name; //目标名字
string msg_name;
...
void swarm_command_cb(const prometheus_msgs::SwarmCommand::ConstPtr& msg)
{
    ...
    if(Command_Now.Mode == prometheus_msgs::SwarmCommand::Formation_containment)
    {
...
void target_state_cb(const prometheus_msgs::DroneState::ConstPtr& msg)
{
    pos_target = Eigen::Vector3d(msg->position[0], msg->position[1], msg->position[2]);
    vel_target = Eigen::Vector3d(msg->velocity[0], msg->velocity[1], msg->velocity[2]);
}
