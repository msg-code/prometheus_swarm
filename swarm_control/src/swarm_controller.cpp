...
int main(int argc, char **argv)
{
    ...
    // 监听目标的状态信息 后期目标考虑使用人的位置信息
    nh.param<int>("target_id", target_id, 0);
    nh.param<string>("target_name", target_name, "/target0");
    // 控制变量
    nh.param<float>("k_a", k_a, 10);
    nh.param<float>("k_b", k_b, 10);
    ...
    // 【订阅】目标的状态信息
    target_state_sub = nh.subscribe<prometheus_msgs::DroneState>(target_name + "/prometheus/drone_state", 10, target_state_cb);
