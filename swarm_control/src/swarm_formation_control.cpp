...
int main(int argc, char **argv)
{
    ...
    while (ros::ok()) // todo: only check start_flag=0, other function need tested
    {
        ...
        cout << "Please choose the action: 0 for Formation Shape, 1 for Virtual Leader Pos, 2 for Hold, 3 for Land, 5 for Disarm 9 for containment"<<endl;
        cin >> start_flag;
        if (start_flag == 0)
        {
        ...
        else if (start_flag == 5)
        {
            ...
            for(int i = 1; i <= swarm_num; i++) 
            {
                swarm_command[i].Mode = prometheus_msgs::SwarmCommand::Formation_containment;
                command_pub[i].publish(swarm_command[i]); //【发布】阵型
            }
        }else
        {
